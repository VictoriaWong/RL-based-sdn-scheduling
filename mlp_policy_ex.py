...
class MlpPolicy(object):
    ...
    def _init(self, vf_ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        # assert isinstance(ac_ob_space, gym.spaces.Box)

        self.pdtype = pdtype = make_pdtype(ac_space)
        sequence_length = None

        vf_ob = U.get_placeholder(name="vf_ob", dtype=tf.float32, shape=[sequence_length]+list(vf_ob_space.shape))
        # ac_ob = U.get_placeholder(name="ac_ob", dtype=tf.float32, shape=[sequence_length]+list(ac_ob_space.shape))
        ...
        with tf.variable_scope('vf'):
            ...
            for i in range(num_hid_layers):
                vf_last_out = tf.nn.tanh(tf.layers.dense(vf_last_out, hid_size, name="fc%i" % (i+1), kernel_initializer=U.normc_initializer(1.0)))
            self.vpred = tf.layers.dense(vf_last_out, 1, name='final', kernel_initializer=U.normc_initializer(1.0))[:,0]

        with tf.variable_scope('pol'):
            # ac_obz = tf.clip_by_value((nn_in - self.nn_in_rms.mean) / self.nn_in_rms.std, -5.0, 5.0)
            ac_last_out = nn_in
            for i in range(num_hid_layers):
                ac_last_out = tf.nn.tanh(tf.layers.dense(ac_last_out, hid_size, name='fc%i' % (i + 1), kernel_initializer=U.normc_initializer(1.0)))
            self.nn_out = tf.nn.softplus(tf.layers.dense(ac_last_out, 1, name='mid_final', kernel_initializer=U.normc_initializer(1.0)))[:,0]  # action value prediction/ performance of each controller
